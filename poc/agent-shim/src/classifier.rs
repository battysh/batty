...
const CLAUDE_PROMPT_CHARS: &[char] = &['❯'];
...
fn looks_like_claude_spinner(line: &str) -> bool {
    ...
    CLAUDE_SPINNER_CHARS.contains(&first) && (trimmed.contains('…') || trimmed.contains("(thinking"))
}
    ...
    for line in &recent_nonempty {
        ...
        if trimmed.starts_with('›')
            && (trimmed.len() <= '›'.len_utf8()
                || trimmed['›'.len_utf8()..].starts_with(|c: char| c.is_whitespace()))
        {
            return ScreenVerdict::AgentIdle;
        }
    }
...
mod tests {
    ...
    fn claude_idle_prompt() {
        ...
        assert_eq!(classify(AgentType::Claude, parser.screen()), ScreenVerdict::AgentIdle);
    }

    #[test]
    fn claude_working_spinner() {
        let parser = make_screen("· Thinking…\n");
        assert_eq!(
    ...
    fn codex_idle_prompt() {
        ...
        assert_eq!(classify(AgentType::Codex, parser.screen()), ScreenVerdict::AgentIdle);
    }
    ...
    fn kiro_idle_prompt() {
        ...
        assert_eq!(classify(AgentType::Kiro, parser.screen()), ScreenVerdict::AgentIdle);
    }
    ...
    fn exhaustion_all_types() {
        for agent_type in [AgentType::Claude, AgentType::Codex, AgentType::Kiro, AgentType::Generic] {
