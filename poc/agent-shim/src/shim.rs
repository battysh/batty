...
use crate::classifier::{self, AgentType, ScreenVerdict};
use crate::protocol::{Channel, Command, Event, ShimState};
...
pub fn run(args: ShimArgs, channel: Channel) -> Result<()> {
    ...
    let pty_handle = std::thread::spawn(move || {
            ...
            match pty_reader.read(&mut buf) {
                ...
                Ok(n) => {
                    ...
                    let new_state = match (old_state, verdict) {
                        ...
                        // Unknown doesn't change state
                        (_, ScreenVerdict::Unknown) => None,
                        // AgentWorking while idle shouldn't normally happen,
                        // but if it does, respect the classifier
                    ...
                    if let Some(new) = new_state {
                        ...
                        // Build events to send (release lock before sending)
        ...
        // portable-pty's wait may block; we already know PTY is closed
    ...
    loop {
        ...
        match state {
            ...
            ShimState::Dead => {
                // Agent died before showing prompt
                return Ok(());
            }
            _ => {
                // Ready! Send Ready event.
    ...
    loop {
        let cmd = match cmd_channel.recv::<Command>() {
            Ok(Some(c)) => c,
            Ok(None) => {
                // Orchestrator closed connection — shut down
        ...
        match cmd {
            Command::SendMessage {
                ...
                match inner.state {
                    ...
                    ShimState::Working => {
                        // Agent busy — reject (orchestrator should queue externally)
                        cmd_channel.send(&Event::Error {
                            command: "SendMessage".into(),
            ...
            Command::Resize { rows, cols } => {
                ...
                drop(inner);
                // No explicit ack — state will reflect the new size
            }
            ...
            Command::Shutdown { timeout_secs } => {
                eprintln!("[shim {}] shutdown requested (timeout: {}s)", args.id, timeout_secs);
                // Send Ctrl-C then wait
                {
                    let mut writer = pty_writer.lock().unwrap();
...
/// screen content. Simple approach: take lines in current that weren't in pre.
fn extract_response(pre: &str, current: &str) -> String {
    let pre_lines: Vec<&str> = pre.lines().collect();
    let cur_lines: Vec<&str> = current.lines().collect();

    // Find where new content starts (after the pre-injection content)
    ...
    // Everything from diverge point to end (minus the last prompt line)
    ...
    // Strip trailing prompt lines (there may be multiple)
    while end > 0 && is_prompt_line(response_lines[end - 1].trim()) {
        end -= 1;
    }
    // Strip any trailing empty lines exposed by removing the prompt
    while end > 0 && response_lines[end - 1].trim().is_empty() {
        end -= 1;
    }
