...
pub fn engineer_workspace_dir(
    ...
    match workspace_type {
        ...
        WorkspaceType::workspace => workspace_workspace_src_dir(project_root, engineer),
    }
}

pub fn workspace_workspace_root(project_root: &Path, engineer: &str) -> PathBuf {
    project_root
        .parent()
        .unwrap_or(project_root)
        .join(".batty-workspace")
        .join(engineer)
}

pub fn workspace_workspace_src_dir(project_root: &Path, engineer: &str) -> PathBuf {
    workspace_workspace_root(project_root, engineer).join("src")
}

pub fn remove_empty_workspace_workspace_root(project_root: &Path, engineer: &str) -> Result<()> {
    let root = workspace_workspace_root(project_root, engineer);
...
pub fn setup_workspace_worktree(
    ...
    register_workspace_workspace_if_needed(workspace_type, worktree_dir, sub_repo_names)?;
...
pub fn prepare_workspace_assignment_worktree(
    ...
    register_workspace_workspace_if_needed(
...
fn register_workspace_workspace_if_needed(
    workspace_type: WorkspaceType,
    workspace_src_dir: &Path,
    sub_repo_names: &[String],
) -> Result<()> {
    if !workspace_type.is_workspace() {
        return Ok(());
    }
    ...
    if !workspace_available() {
        warn!(
            ...
            "workspace workspace registration skipped because `workspace` is unavailable"
        );
        return Ok(());
    }
    ...
    for repo_name in sub_repo_names {
        let output = Command::new("workspace")
            ...
            .with_context(|| {
                format!(
                    "failed to register workspace package '{repo_name}' in {}",
                    workspace_src_dir.display()
                )
            })?;
        if !output.status.success() {
            anyhow::bail!(
                "workspace registration failed for package '{}' in {}: {}",
                repo_name,
                workspace_src_dir.display(),
...
mod tests {
    ...
    #[test]
    fn workspace_workspace_dir_is_sibling_workspace_src() {
        let project_root = Path::new("/ws/src");

        assert_eq!(
            engineer_workspace_dir(project_root, WorkspaceType::workspace, "eng-1"),
            PathBuf::from("/ws/.batty-workspace/eng-1/src")
        );
    }
